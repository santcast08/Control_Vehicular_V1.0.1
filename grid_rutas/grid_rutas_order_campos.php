...
<?php
   ...
   $Ord_Cmp = new grid_rutas_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_rutas_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_def_campos['id_rutas'] = "Id_Rutas";
   $tab_converte["Id_Rutas"]   = "id_rutas";
   $tab_labels["id_rutas"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["id_rutas"])) ? $_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["id_rutas"] : "Id Rutas";
   $tab_ger_campos['rutas_origen'] = "on";
   $tab_def_campos['rutas_origen'] = "rutas_Origen";
   $tab_converte["rutas_Origen"]   = "rutas_origen";
   $tab_labels["rutas_origen"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_origen"])) ? $_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_origen"] : "Rutas Origen";
   $tab_ger_campos['rutas_destino'] = "on";
   $tab_def_campos['rutas_destino'] = "rutas_Destino";
   $tab_converte["rutas_Destino"]   = "rutas_destino";
   $tab_labels["rutas_destino"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_destino"])) ? $_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_destino"] : "Rutas Destino";
   $tab_ger_campos['rutas_distancia'] = "on";
   $tab_def_campos['rutas_distancia'] = "Rutas_Distancia";
   $tab_converte["Rutas_Distancia"]   = "rutas_distancia";
   $tab_labels["rutas_distancia"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_distancia"])) ? $_SESSION['sc_session'][$sc_init]['grid_rutas']['labels']["rutas_distancia"] : "Rutas Distancia";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_rutas']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_rutas']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select'][$campo] = $ordem;
       }
   }
?>
    <script language="javascript"> 
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_rutas_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_rutas']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
