...
$old_error_handler = set_error_handler("form_preventivo_scriptcase_error_handler");

function form_preventivo_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_preventivo']['contr_erro']) && $_SESSION['scriptcase']['form_preventivo']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_preventivo->contr_form_preventivo->Apl_com_erro = true;
            if ($inicial_form_preventivo->contr_form_preventivo->NM_ajax_flag)
            {
                $inicial_form_preventivo->contr_form_preventivo->NM_ajax_info['errList']['geral_form_preventivo'][] = $msg;
                return;
            }
            form_preventivo_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_preventivo_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_preventivo'])  && '' != $_SESSION['scriptcase']['error_icon']['form_preventivo'])  ? $_SESSION['scriptcase']['error_icon']['form_preventivo']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_preventivo']) && '' != $_SESSION['scriptcase']['error_close']['form_preventivo']) ? $_SESSION['scriptcase']['error_close']['form_preventivo'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_preventivo']) && '' != $_SESSION['scriptcase']['error_span_title']['form_preventivo']) ? $_SESSION['scriptcase']['error_span_title']['form_preventivo'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_preventivo']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_preventivo']) ? $_SESSION['scriptcase']['error_icon_title']['form_preventivo'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_preventivo']['error_buffer'] .= ob_get_flush();
}
