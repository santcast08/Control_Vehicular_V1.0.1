...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["km_galon" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["km_galon" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["km_galon" + iSeqRow]["change"]) {
    return true;
  }
...
function scEventControl_onFocus(oField, iSeq) {
  ...
  if ("idvehiculo" + iSeq == fieldName) {
    scEventControl_data[fieldName]["blur"] = false;
  }
  if ("km_galon" + iSeq == fieldName) {
    scEventControl_data[fieldName]["blur"] = false;
  }
  if ("movilizacion_funcionario" + iSeq == fieldName) {
    scEventControl_data[fieldName]["blur"] = false;
  }
...
function scJQEventsAdd(iSeqRow) {
  ...
  $('#id_sc_field_movilizacion_funcionario' + iSeqRow).bind('blur', function() { sc_form_movilizacion_movilizacion_funcionario_onblur(this, iSeqRow) })
                                                      .bind('focus', function() { sc_form_movilizacion_movilizacion_funcionario_onfocus(this, iSeqRow) });
  ...
  $('#id_sc_field_km_galon' + iSeqRow).bind('blur', function() { sc_form_movilizacion_km_galon_onblur(this, iSeqRow) })
                                      .bind('focus', function() { sc_form_movilizacion_km_galon_onfocus(this, iSeqRow) });
} // scJQEventsAdd
...
function sc_form_movilizacion_km_galon_onblur(oThis, iSeqRow) {
  do_ajax_form_movilizacion_mob_validate_km_galon();
  scCssBlur(oThis);
}

function sc_form_movilizacion_km_galon_onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis);
}
...
function displayChange_block_0(status) {
	...
	displayChange_field("Km_galon", "", status);
}
...
function displayChange_row(row, status) {
	...
	displayChange_field_km_galon(row, status);
...
function displayChange_field_idusuario(row, status) {
	if ("on" == status) {
		$("#id_sc_field_idusuario" + row).select2("destroy");
		scJQSelect2Add(row, "idusuario");
	}
}
...
function displayChange_field_km_galon(row, status) {
}

function displayChange_field_movilizacion_funcionario(row, status) {
	if ("on" == status) {
		$("#id_sc_field_movilizacion_funcionario" + row).select2("destroy");
		scJQSelect2Add(row, "movilizacion_funcionario");
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_movilizacion_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_movilizacion_fecha" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_movilizacion_mob_validate_movilizacion_fecha(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idusuario" == specificField) {
    scJQSelect2Add_idusuario(seqRow);
  }
  if (null == specificField || "movilizacion_funcionario" == specificField) {
    scJQSelect2Add_movilizacion_funcionario(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idusuario(seqRow) {
  $("#id_sc_field_idusuario" + seqRow).select2(
    {
      language: {
        noResults: function() {
...
function scJQSelect2Add_movilizacion_funcionario(seqRow) {
  $("#id_sc_field_movilizacion_funcionario" + seqRow).select2(
    {
      language: {
        noResults: function() {
          return "<?php echo $this->Ini->Nm_lang['lang_autocomp_notfound'] ?>";
