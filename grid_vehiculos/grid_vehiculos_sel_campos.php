...
<?php
     ...
     if (!function_exists("NM_is_utf8"))
     {
      ...
      $Sel_Cmp = new grid_vehiculos_sel_cmp($bol_sel_campos_include); 
      $Sel_Cmp->Sel_cmp_init();
      $Sel_Cmp->Sel_cmp_init_fields();
      $Sel_Cmp->Sel_cmp_process();
     
  }
class grid_vehiculos_sel_cmp
{
...
function Sel_cmp_process()
{
   ...
   function Sel_cmp_init_fields()
   {
      ...
      $tab_blk_campos[] = "usuario_idusuario";
      ...
      $tab_ger_campos['vehiculo_matricula'] = "on";
      ...
      if (isset($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['php_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
              if ($NM_cada_opc == "off")
              {
                 $tab_ger_campos[$NM_cada_field] = "none";
      ...
      if (isset($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel'] = array();
   }
   if($this->bol_sel_campos_include)
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['export_sel_columns']['field_order'] = $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'];
       $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['export_sel_columns']['usr_cmp_sel'] = $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel'];
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['usr_cmp_sel'] = $arr_temp;
   if($this->bol_sel_campos_include)
   {
       return;
   }
...
?>
  function scSubmitSelCampos(sPos) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_vehiculos_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['labels'][$NM_cada_field];
           }
       }
   }
...
<?php
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['field_order'] as $NM_cada_field)
       {
...
?>
                        <li class="sc_ui_litem <?php echo $str_class; ?>" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_vehiculos']['labels'][$NM_cada_field]; ?></li>
<?php
           }
       }
   }
